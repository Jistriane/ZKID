...
describe('Project Starter Character Plugin Ordering', () => {
  ...
  describe('Plugin Categories and Ordering', () => {
    it('should categorize plugins correctly', () => {
      ...
      const embeddingPlugins = ['@elizaos/plugin-openai', '@elizaos/plugin-ollama', '@elizaos/plugin-google-genai']

      // Platform plugins
      const platformPlugins = ['@elizaos/plugin-discord', '@elizaos/plugin-twitter', '@elizaos/plugin-telegram']
  ...
  describe('Plugin Logic Validation', () => {
    it('should follow the expected plugin ordering pattern', () => {
      ...
      const textOnlyIndices = textOnlyPlugins.map((p) => plugins.indexOf(p)).filter((i) => i !== -1)
      const embeddingIndices = embeddingPlugins.map((p) => plugins.indexOf(p)).filter((i) => i !== -1)
  ...
  describe('Complex Configuration Scenarios', () => {
    it('should handle complete AI provider setup correctly', () => {
      ...
      const hasOtherAiProviders = character.plugins.some((plugin) => allAiProviders.includes(plugin))
